const attach = nodeList => {
    ...
    // HTML
    nodeList.textContent = input => {
        for (node of nodeList) node.textContent = input;
    };
    nodeList.text = input => {
        for (node of nodeList) node.innerText = input;
    };
    nodeList.html = input => {
    ...
    nodeList.css = (...args) => {
        if (typeof args[0] == "object" && args.length == 1)
            ...
            for (node of nodeList)
                return `${args}: ${window.getComputedStyle(node)[args] || "Not defined"}`;
    };
    nodeList.hide = () => {
        for (node of nodeList) {
...
const µ = (...args) => {
    if (typeof args[0] === "function") {
        ...
        doc.ajax = (url, options, responseFct, dataFct, catchFct) => {
            return fetch(url, options).then(responseFct).then(dataFct).catch(catchFct);
            /*
